...
export const SUMMARY_INSTRUCTION_BASE = `
You are an expert sales analyst AI. Your task is to analyze the following chat transcript between a potential client and an AI assistant, then return a single, minified JSON object with NO other text before or after it.
...
- budgetMention: (String) Extract any mention of budget, even if vague. If none, use an empty string.
- timelineMention: (String) Extract any mention of a timeline or urgency. If none, use an empty string.
`;
...
export const SUMMARY_INSTRUCTION_ES = `${SUMMARY_INSTRUCTION_BASE}\n\nLos valores JSON (como el texto de resumen y las etiquetas) deben estar en español.`;
export const SUMMARY_INSTRUCTION_PT = `${SUMMARY_INSTRUCTION_BASE}\n\nOs valores JSON (como o texto do resumo e as tags) devem estar em português.`;
